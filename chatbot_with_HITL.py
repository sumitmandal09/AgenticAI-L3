...
# Override with:  OLLAMA_MODEL=gpt-oss:120b-cloud python chatbot_with_hitl.py
...
def purchase_stock(symbol: str, quantity: int) -> dict:
    ...
    decision = interrupt(f"Approve buying {quantity} shares of {symbol}? (yes/no)")     #Here we have used the interrupt function to pause the graph and wait for human input. The message passed to interrupt will be displayed to the user, prompting them for a decision.
...
#Go to Terminal and run this to activate env:   .venv\Scripts\activate.bat
# Then run:  uv run chatbot_with_HITL.py
